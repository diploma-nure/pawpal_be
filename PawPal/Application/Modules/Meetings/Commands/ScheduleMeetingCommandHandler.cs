    ...
    : IRequestHandler<ScheduleMeetingCommand, int>
{
    ...
    public async Task<int> Handle(ScheduleMeetingCommand command, CancellationToken cancellationToken)
    {
        ...
        if (application.Status is not ApplicationStatus.MeetingApproved && application.Status is not ApplicationStatus.MeetingScheduled)
            throw new ConflictException(Constants.ResponseCodes.ConflictMeetingScheduleForbiddenForApplicationStatus, $"Application must be in status {ApplicationStatus.MeetingApproved} or {ApplicationStatus.MeetingScheduled} to schedule a meeting");

        var currentDate = DateTime.UtcNow;
        if (application.Meeting is not null)
        {
            if (application.Meeting.End < currentDate)
                throw new ConflictException(Constants.ResponseCodes.ConflictMeetingRescheduleForbiddenForMeetingStatus, "Meeting for this application cannot be rescheduled");
        ...
        if (start.TimeOfDay < workDayStartTime.ToTimeSpan() || end.TimeOfDay > workDayEndTime.ToTimeSpan())
            throw new ConflictException(Constants.ResponseCodes.ConflictBadScheduleTime, "Meeting is out of scheduled working time");

        if (start.Date < currentDate.Date || (start.Date.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday))
            throw new ConflictException(Constants.ResponseCodes.ConflictBadScheduleDate, "Unable to schedule a meeting for this date");

        if (start.Date == currentDate.Date && start.TimeOfDay < currentDate.TimeOfDay)
            throw new ConflictException(Constants.ResponseCodes.ConflictBadScheduleTime, "Unable to schedule a meeting for this time");
        ...
        var availableAdmin = await _dbContext.Users
            ...
            ?? throw new ConflictException(Constants.ResponseCodes.ConflictNoAvailableAdmins, "No available admins for the selected time slot");
